...
def list_available_configs():
    ...
    Returns:
        list: List of tuples containing (config_name, description, file_path)
    """
    ...
    for config_file in config_dir.glob("*.yml"):
        ...
        try:
            ...
            retriever_type = retrieval_config.get(
            ...
            configs.append(
                (config_name, f"Error reading config: {e}", str(config_file)))
...
def switch_agent_config(config_name: str):
    ...
    Args:
        config_name (str): Name of the configuration to switch to
    ...
    try:
        ...
        print(
            f"   Retriever: {retriever_info.get('type', 'unknown')} (top_k={retriever_info.get('top_k', 5)})")
...
def main():
    """
    Main function to handle command line arguments and execute configuration switching.
    """
    parser = argparse.ArgumentParser(
        description="Switch agent retrieval configuration")
    parser.add_argument("config_name", nargs='?',
                        help="Name of the configuration to switch to")
