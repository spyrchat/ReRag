...
def inspect_collections(client: QdrantClient):
    ...
    for collection in collections.collections:
        ...
        print(f"   Vectors: {info.vectors_count if info.vectors_count else 'Computing...'}")
        
        # Vector configuration
        if hasattr(info.config.params, 'vectors'):
            if isinstance(info.config.params.vectors, dict):
                for name, config in info.config.params.vectors.items():
                    print(f"   Vector '{name}': {config.size}D, distance={config.distance}")
            else:
                print(f"   Vector: {info.config.params.vectors.size}D, distance={info.config.params.vectors.distance}")
...
def browse_data(client: QdrantClient, collection_name: str, limit: int = 10):
    ...
    try:
        ...
        for i, point in enumerate(points, 1):
            ...
            print(f"   Chunk: {payload.get('chunk_index', 0)}/{payload.get('num_chunks', 1)}")
...
def collection_stats(client: QdrantClient, collection_name: str):
    ...
    try:
        ...
        print(f"📊 Average text length: {total_chars / len(all_points):.1f} characters")
...
def main():
    ...
    # Browse data
    browse_parser = subparsers.add_parser("browse", help="Browse collection data")
    browse_parser.add_argument("collection", help="Collection name")
    browse_parser.add_argument("--limit", type=int, default=10, help="Number of documents to show")
    ...
    filter_parser.add_argument("--limit", type=int, default=10, help="Number of results")
    ...
    try:
        ...
        if args.command == "list":
            ...
            filter_by_metadata(client, args.collection, args.key, args.value, args.limit)
