...
class QdrantHybridRetriever(ModernBaseRetriever):
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            if 'dense' in embedding_config and 'sparse' in embedding_config:
                ...
                self.sparse_embedding = get_embedder(embedding_config['sparse'])
            else:
            ...
            sparse_vector_name = qdrant_config.get('sparse_vector_name', 'sparse')
            ...
            try:
                ...
                logger.warning("Hybrid mode not available, falling back to dense mode")
            ...
            logger.error(f"Failed to initialize hybrid retriever components: {e}")
            self._initialized = False
    ...
    def _perform_search(self, query: str, k: int) -> List[RetrievalResult]:
        ...
        if not self._initialized:
            logger.warning("Hybrid retriever not properly initialized, returning empty results")
            return []
    ...
    def _perform_separate_searches(self, query: str, k: int) -> Dict[str, List[RetrievalResult]]:
        ...
        try:
            ...
            dense_config['embedding'] = self.config['embedding']['dense'] if 'dense' in self.config.get('embedding', {}) else self.config.get('embedding', {})
            ...
            sparse_config['embedding'] = self.config['embedding']['sparse'] if 'sparse' in self.config.get('embedding', {}) else self.config.get('embedding', {})
            ...
            logger.warning(f"Separate searches failed, falling back to Qdrant hybrid: {e}")
