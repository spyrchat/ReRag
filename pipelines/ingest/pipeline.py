...
class IngestionPipeline:
    ...
    def ingest_dataset(
        ...
        logger.info(
            f"Starting ingestion: {adapter.source_name} v{adapter.version} (split: {split.value})")
        if dry_run:
        ...
        try:
            ...
            documents = self._read_and_validate_documents(
            ...
            logger.info(
                f"Ingestion completed: {record.successful_chunks} successful, {record.failed_chunks} failed")
            return record
    ...
    def _read_and_validate_documents(
        ...
        # Read rows from adapter (don't limit here - let adapter process all rows for proper mapping)
        rows = list(adapter.read_rows(split))

        # Convert to documents
        documents = adapter.to_documents(rows, split)

        # Apply max_documents limit AFTER document creation
        if self.max_documents:
            documents = documents[:self.max_documents]
            logger.info(
                f"Limited to {self.max_documents} documents for testing")

        record.total_documents = len(documents)

        logger.info(
            f"Read {len(documents)} documents from {adapter.source_name}")

        # Validate documents
        print("✅ Validating documents...")
        validation_results = []
        for doc in tqdm(documents, desc="Validating documents", unit="doc"):
        ...
        print("🧹 Cleaning valid documents...")
        for doc, validation_result in tqdm(zip(documents, validation_results),
                                           desc="Processing validation",
                                           total=len(documents), unit="doc"):
        ...
        logger.info(
            f"Validation: {len(valid_documents)} valid, {len(validation_errors)} invalid")

        if validation_errors:
            # Show first 5
            logger.warning(f"Validation errors found: {validation_errors[:5]}")
    ...
    def _chunk_documents(self, documents: List[Any], record: IngestionRecord) -> List[Any]:
        ...
        if strategy_name == "auto":
            ...
            strategy = ChunkingStrategyFactory.get_strategy_for_content(
                sample_content, chunking_config)
        else:
            strategy = ChunkingStrategyFactory.create_strategy(
                strategy_name, chunking_config)
        ...
        print("📊 Updating chunk metadata...")
        doc_chunk_counts = {}
        for chunk in tqdm(chunks, desc="Counting chunks per doc", unit="chunk"):
            doc_id = chunk.metadata.get("external_id", "unknown")
            doc_chunk_counts[doc_id] = doc_chunk_counts.get(doc_id, 0) + 1

        # Update num_chunks for each chunk
        for chunk in tqdm(chunks, desc="Updating metadata", unit="chunk"):
            doc_id = chunk.metadata.get("external_id", "unknown")
    ...
    def _process_chunks(self, chunks: List[Any], record: IngestionRecord) -> List[ChunkMeta]:
        ...
        record.sample_doc_ids = list(
            set(meta.doc_id for meta in chunk_metas[:10]))
        record.sample_chunk_ids = [meta.chunk_id for meta in chunk_metas[:10]]

        logger.info(
            f"Processed {len(chunk_metas)} chunk metas with embeddings")
        return chunk_metas
    ...
    def _upload_chunks(self, chunk_metas: List[ChunkMeta]) -> IngestionRecord:
        ...
        if self.canary_mode:
            ...
            logger.info(
                f"Using canary collection: {self.uploader.collection_name}")

        print(f"📤 Uploading {len(chunk_metas)} chunks to vector store...")
    ...
    def _save_lineage(self, record: IngestionRecord):
        ...
        lineage_file = self.lineage_dir / \
            f"{record.dataset_name}_{record.run_id}.json"
    ...
    def cleanup_canary_collections(self):
        ...
        canary_collections = [
            c.name for c in collections if "_canary" in c.name]
        ...
        for collection_name in canary_collections:
            try:
                ...
                logger.error(
                    f"Error deleting canary collection {collection_name}: {e}")
    ...
    def ingest_multiple_datasets(
        ...
        **kwargs
        ...
        for i, adapter in enumerate(adapters):
            logger.info(
                f"Processing dataset {i+1}/{len(adapters)}: {adapter.source_name}")
        ...
        logger.info(
            f"Batch ingestion completed: {len(self.results)} datasets processed")
        return self.results
