...
class EmbeddingPipeline:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.embedding_strategy in ["dense", "hybrid"]:
            ...
            logger.info(
                f"Initialized dense embedder: {dense_config.get('provider', 'unknown')}")
        ...
        if self.embedding_strategy in ["sparse", "hybrid"]:
            ...
            logger.info(
                f"Initialized sparse embedder: {sparse_config.get('provider', 'unknown')}")

        # Caching configuration
        self.enable_cache = config.get(
            "embedding_cache", {}).get("enabled", True)
        self.cache_dir = Path(config.get(
            "embedding_cache", {}).get("dir", "cache/embeddings"))
        ...
        self.fallback_embedding_dim = config.get(
            "embedding", {}).get("fallback_dim", 384)
    ...
    def process_documents(self, documents: List[Document]) -> List[ChunkMeta]:
        ...
        logger.info(
            f"Processing {len(documents)} documents with {self.embedding_strategy} strategy")

        chunk_metas = []

        # Convert documents to ChunkMeta objects
        print("📄 Converting documents to chunks...")
        for doc in tqdm(documents, desc="Converting documents", unit="doc"):
        ...
        if self.dense_embedder:
            ...
            print("🔗 Attaching dense embeddings...")
            for meta, embedding in tqdm(zip(chunk_metas, dense_embeddings),
                                        desc="Attaching dense embeddings",
                                        total=len(chunk_metas), unit="chunk"):
                meta.dense_embedding = embedding
                if embedding:
                    meta.embedding_dim = len(embedding)
                    meta.embedding_model = self._get_model_name(
                        self.dense_embedder)

        if self.sparse_embedder:
            print("🕸️  Generating sparse embeddings...")
            ...
            print("🔗 Attaching sparse embeddings...")
            for meta, embedding in tqdm(zip(chunk_metas, sparse_embeddings),
                                        desc="Attaching sparse embeddings",
                                        total=len(chunk_metas), unit="chunk"):
    ...
    def _generate_embeddings(self, texts: List[str], embedder: Embeddings, embedding_type: str) -> List[Optional[Union[List[float], Dict[int, float]]]]:
        ...
        with tqdm(total=total_batches, desc=f"Processing {embedding_type} batches", unit="batch") as pbar:
            for i in range(0, len(texts), self.batch_size):
                batch_texts = texts[i:i + self.batch_size]
                batch_embeddings = self._process_embedding_batch(
                ...
                cached_count = sum(
                    1 for e in batch_embeddings if e is not None)
                pbar.set_postfix({
                    'texts': f"{min(i + self.batch_size, len(texts))}/{len(texts)}",
                    'cached': f"{cached_count}/{len(batch_embeddings)}"
                })
    ...
    def _process_embedding_batch(self, texts: List[str], embedder: Embeddings, embedding_type: str) -> List[Optional[Union[List[float], Dict[int, float]]]]:
        ...
        for text in tqdm(texts, desc=f"Embedding {embedding_type}", unit="text", leave=False):
            try:
                # Check cache first
                if self.enable_cache:
                    cached_embedding = self._get_cached_embedding(
                        text, embedder, embedding_type)
                ...
                if self.enable_cache and embedding is not None:
                    self._cache_embedding(
                        text, embedding, embedder, embedding_type)

                batch_embeddings.append(embedding)

            except Exception as e:
                logger.error(
                    f"Error generating {embedding_type} embedding for text: {text[:100]}... Error: {e}")
                ...
                if embedding_type == "dense":
                    ...
                    if embedding is not None:
                        if isinstance(embedding, dict) and len(embedding) > 0:
                            ...
                            logger.warning(
                                f"Converting sparse embedding type {type(embedding)} to dict")
                            return dict(embedding) if hasattr(embedding, 'items') else {}
                        else:
                            logger.warning(
                                f"Unexpected sparse embedding type: {type(embedding)}")
                            return {}

                logger.warning(
                    f"Empty or invalid {embedding_type} embedding returned for text: {text[:50]}...")
                return None

            except Exception as e:
                logger.warning(
                    f"Attempt {attempt + 1}/{self.max_retries} failed for {embedding_type} embedding: {e}")
        ...
        try:
            ...
            if embedding_type == "sparse" and isinstance(embedding, dict):
                serializable_embedding = {
                    str(k): v for k, v in embedding.items()}
