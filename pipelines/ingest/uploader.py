...
class VectorStoreUploader:
    ...
    def _chunk_meta_to_point(self, meta: ChunkMeta) -> Optional[PointStruct]:
        ...
        try:
            ...
            dense_embedding = meta.metadata.get("dense_embedding")
            sparse_embedding = meta.metadata.get("sparse_embedding")
            ...
            return PointStruct(
                id=meta.chunk_id,  # Use deterministic chunk_id
                vector=vectors,
                payload=payload
            )
    ...
    def _ensure_collection_exists(self, chunk_metas: List[ChunkMeta]):
        ...
        if any(meta.metadata.get("dense_embedding") for meta in chunk_metas):
        ...
        if any(meta.metadata.get("sparse_embedding") for meta in chunk_metas):
