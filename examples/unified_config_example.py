...
def main():
        ...
        try:
            ...
            if isinstance(embedding, dict):
                if "provider" in embedding:
                    print(f"   📡 Embedding: {embedding.get('provider')} - {embedding.get('model', 'N/A')}")
                elif "dense" in embedding:
                    dense = embedding.get("dense", {})
                    print(f"   📡 Dense Embedding: {dense.get('provider')} - {dense.get('model', 'N/A')}")
                    sparse = embedding.get("sparse", {})
                    print(f"   📡 Sparse Embedding: {sparse.get('provider')} - {sparse.get('model', 'N/A')}")
    ...
    try:
        ...
        hybrid_pipeline = RetrievalPipelineFactory.create_from_unified_config(config, "hybrid")
        print(f"✅ Created hybrid pipeline with {len(hybrid_pipeline.components)} components")
        
        # Create dense pipeline
        dense_pipeline = RetrievalPipelineFactory.create_from_unified_config(config, "dense")
        print(f"✅ Created dense pipeline with {len(dense_pipeline.components)} components")
        
        # Create default pipeline (should use hybrid)
        default_pipeline = RetrievalPipelineFactory.create_from_unified_config(config)
        print(f"✅ Created default pipeline with {len(default_pipeline.components)} components")
