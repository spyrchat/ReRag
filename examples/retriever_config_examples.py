...
def example_basic_config_usage():
    ...
    try:
        ...
        logger.info(
            f"Dense retriever config loaded: {dense_config['retriever']['type']}")

        hybrid_config = load_retriever_config("hybrid")
        logger.info(
            f"Hybrid retriever config loaded: {hybrid_config['retriever']['type']}")
...
def example_create_pipeline_from_config():
    ...
    try:
        ...
        pipeline = RetrievalPipelineFactory.create_from_retriever_config(
            "hybrid")
        logger.info(
            f"Created pipeline with {len(pipeline.components)} components")
        logger.info(
            f"Component names: {[comp.component_name for comp in pipeline.components]}")
...
def example_config_validation():
    ...
    for retriever_type, config in all_configs.items():
        try:
            ...
            logger.info(
                f"✓ {retriever_type} config is valid: {retriever.component_name}")
...
def example_custom_config_directory():
    ...
    try:
        ...
        logger.info(
            "Custom config directory example (would load from custom path)")
...
def example_runtime_config_modification():
    ...
    try:
        ...
        logger.info(
            f"Created retriever with modified config: top_k={config['retriever']['top_k']}")
